...
#Assembly
AS=/cluster/home/s_lchueca/engraulis_encrasicolus_genome/00.2_rna_ena/trinity/ena_enen_transcriptome.fa
#Input directory
...
for i in $(find "$IN" -name "*.gz" | sed "s|"$IN"\/||" | sed 's/_[^_]*$//'| sed 's/_[^_]*$//'); \
