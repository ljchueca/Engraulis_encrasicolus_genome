...
module load blobtools/1.1.1 ncbi-blast/2.12.0 
...
blastn -task megablast -query ${AS} -db ${DT} -outfmt '6 qseqid staxids bitscore std' -max_target_seqs 1 -max_hsps 1 -num_threads 96 -evalue 1e-25 -out ${T}.out >${T}.log 2>${T}.err &&

#3. Create a BlobDB
blobtools create -i ${AS} -b ${BA}.cov -t ${T}.out -o ${OUT} >${OUT}.log 2>${OUT}.err  && \
